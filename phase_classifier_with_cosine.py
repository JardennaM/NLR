...
def phase_classifier(url, terms):
    ...
    most_likely_phase = most_likely_phases(indices, terms, text)
...
def determine_likelyhood(i, text, terms):
    """
    ...
    for term in terms:
        sur_text = surrounding_text(i, text, selection=700)
        sur_text_vec = Counter(sur_text)
        term_vect = term_vector(terms[term])
        score[term] = determine_cosine(sur_text_vec, term_vect)
    return score

def most_likely_phases(indices, terms, text):
    likelyhoods_for_word = {}
    for i in indices:
        phase = determine_likelyhood(i, text, terms)
